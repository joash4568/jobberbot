...
# Add Scripts to PATH for Windows Store Python
scripts_path = r"C:\Users\Administrator\AppData\Local\Packages\PythonSoftwareFoundation.Python.3.11_qbz5n2kfra8p0\LocalCache\local-packages\Python311\Scripts"
...
def _get_information() -> dict:
    ...
    if os.environ.get("BOT_TEST_MODE") == "true":
        print(colored("TEST MODE: Using placeholder data...", "yellow"))
        return {
            "full_name": "John Doe",
            ...
            "profiles": [{"network": "LinkedIn", "username": "johndoe", "url": "https://linkedin.com/in/johndoe"}],
            "user_image": None,
            "skills": [{"name": "Writing", "keywords": "Copywriting, SEO, Editing"}, {"name": "Data Entry", "keywords": "Typing, Excel, Accuracy"}],
            "educations": [{"area": "English Literature", "institution": "University of Example", "start_date": "2015-09", "end_date": "2019-05"}],
            "projects": [{"name": "Blog Writing", "description": "Wrote 50+ articles", "keywords": "Writing, SEO", "project_url": "http://example.com"}],
            "work_experience": [{"company": "Tech Corp", "area": "Content Writer", "description": "Wrote tech blogs", "start_date": "2020-01", "end_date": "2022-01"}]
        }
    ...
    user_image_input = input(colored("> What is the URL or path to your Image/Avatar? (Type 'none' to skip): ", "blue"))
    if user_image_input.strip().lower() == "none":
        user_image = None
    else:
...
def _populate_configuration(info) -> None:
    ...
    for edu in info["educations"]:
        education.append({
            ...
            "endDate": edu.get("end_date", "2024-08")   # <= keeps validator happy
        })
    ...
    # =====  skills name must be string, not list  =====
    skills = []
    for s in info["skills"]:
        skills.append({
            "name": s["name"][0] if isinstance(s["name"], list) else s["name"],
            "keywords": s["keywords"]
        })
    # ===================================================
...
def _generate_resume(info: dict) -> str:
    """Generate PDF résumé via resumy; return absolute path or empty string on failure."""
    from termcolor import colored
    ...
    if os.environ.get("BOT_TEST_MODE") == "true":
        ...
        raw_theme = input(colored("> What theme would you like to use? (Default: prairie) ", "blue")).strip()
    theme = raw_theme or "prairie"          # ensure non-empty
    ...
    if response.returncode != 0:
        print(colored("❌  resumy build failed.", "red"))
        return ""

    pdf_path = os.path.abspath("resume.pdf")
    time.sleep(0.5)                # disk flush
    if os.path.isfile(pdf_path):
        print(colored("Resume Generated Successfully!", "green"))
        return pdf_path

    print(colored("❌  resume.pdf not found on disk.", "red"))
    return ""
...
Please generate a professional cover letter for a job application. The cover letter should be
...
The candidate is applying for a remote/work-from-home position. Please emphasize:
1. Reliability and self-discipline in a remote environment.
2. Specific skills relevant to the job description (e.g., typing speed for data entry, research/writing style for writers).
3. A professional yet adaptable tone.
